...
class StudentInvoiceListView(ListView):
    ...
    def get_queryset(self):
        ...
        # ✅ Annotate payable amount = invoice.amount - discount_amount
        queryset = queryset.annotate(
            payable_amount=ExpressionWrapper(
                F("invoice__amount") - F("discount_amount"),
        ...
        if group:
            hsc_ct = ContentType.objects.get_for_model(HscAdmissions)
            hsc_ids = HscAdmissions.objects.filter(add_admission_group=group).values_list("id", flat=True)

            deg_ct = ContentType.objects.get_for_model(DegreeAdmission)
            deg_ids = DegreeAdmission.objects.filter(add_admission_group=group).values_list("id", flat=True)
    ...
    def get_context_data(self, **kwargs):
        ...
        context["groups"] = ['science', 'arts', 'commerce', 'ba', 'bss', 'bbs', 'bsc']
